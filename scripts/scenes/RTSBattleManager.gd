...
func _apply_entity_model(entity: GameEntity) -> bool:
	...
	for child in model_instance.get_children():
		if child is MeshInstance3D and child.mesh:
			var has_vc = child.mesh.get("vertex_color_array") != null if "vertex_color_array" in child.mesh else false
			for si in child.mesh.get_surface_count():
				var m = child.get_active_material(si)
				var mat_class = m.get_class() if m else "null"
				var has_vc_setting = m and "vertex_color_use_as_albedo" in m
				var vc_enabled = m.vertex_color_use_as_albedo if has_vc_setting else false
				if entity.entity_id in [10, 60, 62]:
					print("[VC_ebug] eid=%d mesh=%s mat=%s vc_on_mesh=%s vc_in_mat=%s albedo=%s" % [
						entity.entity_id, child.name, mat_class,
						str(has_vc), str(vc_enabled),
						str(m.albedo_color) if has_vc_setting else "n/a"
					])
				if has_vc_setting and not vc_enabled:
					var dup = m.duplicate()
